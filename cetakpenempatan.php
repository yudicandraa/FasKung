...
$pdf -> cell(220, 5,"PT. ASDP Indonesia Ferry Persero",0,0,'L');

$pdf->SetFont('Arial','',10);
$pdf -> cell(220, 5,"FP 1",0,1,'L');
...
$pdf -> cell(0, 5,"DOKUMEN PENEMPATAN BARANG",0,1,'C');
...
$pdf-> SetFont('Arial','',10);
$no=1;
$tampil = mysqli_query($koneksi, "SELECT * FROM penempatan");
while($hasil = mysqli_fetch_array($tampil)){
    $pdf -> Cell(8, 10,$no++,'LR', 0, 'C');
    $pdf -> Cell(45, 10,$hasil['nama_barang'],'LR', 0, 'C');
    $pdf -> Cell(45, 10,$hasil['merk_barang'],'LR', 0, 'C');
    $pdf -> Cell(35, 10,$hasil['nomor_seri'],'LR', 0, 'C');
    $pdf -> Cell(35, 10,$hasil['nomor_barang'],'LR', 0, 'C');
    $pdf -> Cell(30, 10,$hasil['satuan_jumlah'],'LR', 0, 'C');
    $pdf -> Cell(30, 10,$hasil['jumlah'],'LR', 0, 'C');
    $pdf -> Cell(40, 10,$hasil['keterangan'],'LR', 1, 'C');
}
...
$pdf ->Output('D', 'Laporan Penempatan Barang.pdf');
